...
import search
import pacman
from search import update_exploration_metrics
...
class ExplorationAgent(Agent):
    """
    Exploration-based Pacman agent.
    Prioritizes unvisited positions and minimizes revisits.
    """
    ...
    def getAction(self, state):
        ...
        update_exploration_metrics(self, state)

        # Increment step counter
        self.total_steps += 1

        # Actions leading to unvisited positions
        unvisited_actions = [
            action for action in legal_actions
            if self.getNextPosition(current_position, action) not in self.visited_positions
        ]

        # Prefer exploring unvisited positions
        if unvisited_actions:
            return random.choice(unvisited_actions)

        # Otherwise, choose the least visited position
        return min(
            legal_actions,
            key=lambda action: self.visit_frequency[
                self.getNextPosition(current_position, action)
            ]
        )
    ...
    def final(self, state):
        ...
        repeated_steps = self.total_steps - visited
        repetition_ratio = (
            repeated_steps / self.total_steps if self.total_steps > 0 else 0
        )
        efficiency = (
            100 * visited / self.total_steps if self.total_steps > 0 else 0
        )
        ...
        summary = [
            ...
            f"• Unique positions         : {visited}",
            f"• Repeated steps           : {repeated_steps}",
            f"• Repetition ratio         : {repetition_ratio:.2f}",
            f"• Exploration efficiency   : {efficiency:.1f}%",
            "---------------------------------------------------------------------"
        ]
